...
import { collection, doc, onSnapshot, setDoc, updateDoc, deleteDoc, query, where, getDoc, getDocs, addDoc, serverTimestamp, Timestamp, runTransaction, increment, limit, orderBy } from 'firebase/firestore';
import { ref, set, push, onValue, limitToLast, query as rtdbQuery } from 'firebase/database';
import { auth, db, rtdb } from '../firebase';
import { getThailandNow, getThailandTodayString, formatThailandDate, formatThailandDateTime } from '../utils/dateUtils';
...
function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null, setQuotaExceeded?: (val: boolean) => void) {
  ...
  if (isQuotaError) {
    ...
    safeSetItem('synergy_quota_exceeded_timestamp', String(Date.now()));
  ...
  // If it's a permission error and user is not logged in, it's likely a race condition during sign out
  if (isPermissionError && !auth.currentUser) {
    console.warn('Firestore Permission Error during unauthenticated state (likely sign-out):', path);
    return;
  }

  console.error('Firestore Error: ', JSON.stringify(errInfo));
  
  // Only throw if it's NOT a quota error and NOT a permission error during sign-out
  if (!isQuotaError) {
 ...
 * 
 * 1. Starter Affiliate:
 *    - 5% from Direct Referrals (Default)
 * 
 * 2. Marketer Affiliate:
 *    - 10% from Direct Starter team purchases
 *    - 5% from Indirect Starter team purchases
 *    - 2% from Marketer team (same level, paid for 1 level only)
 * 
 * 3. Builder Affiliate:
 *    - 20% from Direct Starter team purchases
 *    - 15% from Indirect Starter team purchases
 *    - 10% from Direct & Indirect Marketer team purchases
 *    - 2% from Builder team (same level, paid for 1 level only)
 * 
 * 4. Executive Affiliate:
 *    - 30% from Direct Starter team purchases
 *    - 25% from Indirect Starter team purchases
 *    - 20% from Direct & Indirect Marketer team purchases
 *    - 10% from Direct & Indirect Builder team purchases
 *    - 1% from Executive team (same level, paid for 1 level only)
 */
const TIER_RATES = {
...
const DIRECT_RATES = {
  [UserTier.STARTER]: { 
    [UserTier.STARTER]: 0.05, 
    [UserTier.MARKETER]: 0.05, 
    [UserTier.BUILDER]: 0.05, 
    [UserTier.EXECUTIVE]: 0.05 
  },
  [UserTier.MARKETER]: { 
    [UserTier.STARTER]: 0.10, 
    [UserTier.MARKETER]: 0.02,
    [UserTier.BUILDER]: 0,
    [UserTier.EXECUTIVE]: 0
  },
  [UserTier.BUILDER]: { 
    [UserTier.STARTER]: 0.20, 
    [UserTier.MARKETER]: 0.10, 
    [UserTier.BUILDER]: 0.02,
    [UserTier.EXECUTIVE]: 0
  },
  [UserTier.EXECUTIVE]: { 
    [UserTier.STARTER]: 0.30, 
    [UserTier.MARKETER]: 0.20, 
    [UserTier.BUILDER]: 0.10, 
    [UserTier.EXECUTIVE]: 0.01 
  }
...
const INITIAL_PRODUCTS: Product[] = [
  {
    ...
    sold: 120,
    stock: 50,
    description: "A complete set of luxury skincare products for a radiant glow."
  },
  {
    id: 2,
    name: "Advanced Health Supplement",
    price: 1800,
    image: "https://picsum.photos/seed/health/800/800",
    category: "Health",
    sold: 450,
    stock: 100,
    description: "Essential vitamins and minerals for optimal performance."
  },
  {
    id: 3,
    name: "Eco-friendly Smart Bottle",
    price: 950,
    image: "https://picsum.photos/seed/bottle/800/800",
    category: "Lifestyle",
    sold: 85,
    stock: 30,
    description: "Track your hydration with this sustainable smart bottle."
  },
  {
    id: 4,
    ...
    sold: 210,
    stock: 75,
    description: "Clean, plant-based protein for muscle recovery."
  },
  {
    id: 5,
    name: "High-Performance Yoga Mat",
    price: 1500,
    image: "https://picsum.photos/seed/yoga/800/800",
    category: "Lifestyle",
    sold: 300,
    stock: 45,
    description: "Extra grip and cushioning for your daily practice."
  }
];
...
const INITIAL_ADS: Ad[] = [
  {
    ...
    image: "https://picsum.photos/seed/synergy_welcome/1200/600",
    active: true,
  ...
  },
  {
    ...
    image: "https://picsum.photos/seed/sale/1200/600",
    active: true,
    placement: 'home_banner_2'
  }
];
...
const INITIAL_ASSETS: CampaignAsset[] = [
  {
    id: 1,
    title: "Eco Campaign Kit",
    description: "Social media assets for the smart bottle campaign.",
    image: "https://picsum.photos/seed/asset1/800/600",
    active: true,
    category: "Social",
    commission: "5%",
    status: "Active"
  },
  {
    ...
    description: "Banners and captions for health supplements.",
    image: "https://picsum.photos/seed/asset2/800/600",
    active: true,
    category: "Banner",
    commission: "10%",
    status: "Active"
...
const INITIAL_ONBOARDING: OnboardingSlide[] = [
  {
    id: 1,
    title: "Welcome to Synergy Flow",
    desc: "The next generation of affiliate marketing ecosystem.",
    image: "https://picsum.photos/seed/onboarding1/800/800"
  },
  {
    id: 2,
    title: "Build Your Team",
    desc: "Grow your network and earn together with our tiered system.",
    image: "https://picsum.photos/seed/onboarding2/800/800"
  },
  {
    id: 3,
    title: "AI Powered Tools",
    desc: "Generate high-converting content with our AI assistant.",
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (ref) {
      safeSetItem('synergy_referrer_code', ref.toUpperCase());
    }
  }, []);
  ...
  const setNotificationsEnabled = (enabled: boolean) => {
    ...
    safeSetItem('synergy_notifications_enabled', String(enabled));
  };
  ...
  const searchReferrer = async (code: string): Promise<Referrer | null> => {
    try {
      ...
      if (!snapshot.empty) {
        ...
        // Check if the referrer is active (has a referrer themselves or is an admin)
        const isActive = data.referrerCode || data.role === 'admin';
  ...
  const syncPublicProfile = async (u: User, uid: string) => {
    try {
      const publicData = {
        ...
        dailyIncome: u.dailyIncome || 0,
        lastIncomeDate: u.lastIncomeDate || null,
        referrerCode: u.referrerCode || null,
        role: u.role || 'user',
        uid: uid
      };
      // Use merge: true to preserve fields like dailyIncome and lastIncomeDate which are managed separately
      ...
      if (firebaseUser) {
        ...
        unsubUser = onSnapshot(doc(db, 'users', firebaseUser.uid), async (docSnap) => {
          if (docSnap.exists()) {
            ...
            setUser(userData);
            safeSetItem(`synergy_cached_user_${firebaseUser.uid}`, JSON.stringify(userData));
            safeSetItem(`synergy_last_user_doc_fetch_${firebaseUser.uid}`, String(Date.now()));
            
            // Daily Income Reset Check
            const today = getThailandTodayString();

            if (userData.lastIncomeDate && userData.lastIncomeDate !== today && userData.dailyIncome > 0) {
              console.log("Resetting daily income for new day:", today);
              updateDoc(docSnap.ref, {
                dailyIncome: 0,
                lastIncomeDate: today
              }).catch(err => console.error("Failed to reset daily income", err));
              
              updateDoc(doc(db, 'publicProfiles', firebaseUser.uid), {
                dailyIncome: 0,
                lastIncomeDate: today
              }).catch(err => console.error("Failed to reset public daily income", err));
            }
            ...
            let referrerUid = savedReferrerCode ? await getUidByReferralCode(savedReferrerCode) : null;
            let finalReferrerCode = savedReferrerCode;
            ...
            if (referrerUid) {
                ...
                if (referrerDoc.exists()) {
                    ...
                    // Check if referrer is active (has a referrer themselves or is admin)
                    const isReferrerActive = referrerData.role === 'admin' || !!referrerData.referrerCode;
                    
                    if (isReferrerActive) {
                        uplinePath = [...(referrerData.uplinePath || []), referrerUid];
                    } else {
                        // Referrer is not active, ignore this referral
                        referrerUid = null;
            ...
            const newUser: User = {
              ...
              referrerCode: isPrimaryAdmin ? undefined : (finalReferrerCode || undefined), 
            ...
            if (uplinePath.length > 0) {
              for (const ancestorId of uplinePath) {
                await updateDoc(doc(db, 'users', ancestorId), { teamSize: increment(1) });
                await updateDoc(doc(db, 'publicProfiles', ancestorId), { teamSize: increment(1) });
              }
            }
  ...
  useEffect(() => {
    ...
    const unsubProducts = onSnapshot(query(collection(db, 'products'), limit(50)), (snapshot) => {
      ...
      setProducts(data);
      safeSetItem('synergy_cached_products', JSON.stringify(data));
    ...
    const unsubFeed = onSnapshot(query(collection(db, 'feed'), limit(30)), (snapshot) => {
      ...
      setFeed(data);
      safeSetItem('synergy_cached_feed', JSON.stringify(data));
    ...
    const unsubAds = onSnapshot(query(collection(db, 'ads'), limit(10)), (snapshot) => {
      ...
      setAds(data);
      safeSetItem('synergy_cached_ads', JSON.stringify(data));
    ...
    const unsubAssets = onSnapshot(query(collection(db, 'campaignAssets'), limit(20)), (snapshot) => {
      ...
      safeSetItem('synergy_cached_campaignAssets', JSON.stringify(data));
    ...
    const unsubOnboarding = onSnapshot(query(collection(db, 'onboardingSlides'), limit(10)), (snapshot) => {
      ...
      safeSetItem('synergy_cached_onboardingSlides', JSON.stringify(data));
    ...
    const unsubSettings = onSnapshot(doc(db, 'systemSettings', 'current'), (docSnap) => {
      if (docSnap.exists()) {
          ...
          safeSetItem('synergy_cached_settings', JSON.stringify(settings));
      }
  ...
  useEffect(() => {
    ...
    const unsubOrders = onSnapshot(qOrders, (snapshot) => {
      ...
      setAllOrders(data);
      safeSetItem('synergy_cached_orders', JSON.stringify(data));
    }, (err) => handleFirestoreError(err, OperationType.LIST, 'orders', setIsQuotaExceeded));

    const commissionsRef = collection(db, 'commissions');
    const qCommissions = user.role === 'admin'
      ? query(commissionsRef, orderBy('date', 'desc'), limit(200))
      : query(commissionsRef, where('userId', '==', auth.currentUser.uid), orderBy('date', 'desc'), limit(200));

    const unsubCommissions = onSnapshot(qCommissions, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as any;
      setAllCommissions(data);
      safeSetItem('synergy_cached_commissions', JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    socket.on("promotion:stop", handlePromotionStop);
    ...
    return () => {
      ...
      socket.off("promotion:stop", handlePromotionStop);
    };
  ...
  useEffect(() => {
    ...
    getDocs(q).then(async (snapshot) => {
      if (!snapshot.empty) {
        const publicDoc = snapshot.docs[0];
        const publicData = publicDoc.data();
        const uid = publicDoc.id;
        
        // Fetch full contact info from users collection (permission allowed for upline/downline)
        try {
          ...
          if (userDoc.exists()) {
            ...
            setReferrer({
              ...
              lineId: userData.lineId || userData.socials?.line?.name || ''
            });
          } else {
            // Fallback to public data if user doc not accessible
            setReferrer({
              name: publicData.name,
              code: publicData.referralCode,
  ...
  useEffect(() => { safeSetItem('synergy_cart', JSON.stringify(cart)); }, [cart]);
  useEffect(() => { 
    safeSetItem('synergy_lang', language);
    document.documentElement.lang = language;
  }, [language]);
  
  useEffect(() => {
      safeSetItem('synergy_font_size', fontSize);
  ...
  const login = async (emailOrPhone?: string, password?: string, isRegister: boolean = false): Promise<boolean> => {
    ...
    setIsLoggingIn(true);
    if (isRegister) {
      safeSetItem('synergy_is_registering', 'true');
    } else {
      localStorage.removeItem('synergy_is_registering');
    }
    try {
  ...
  const register = async (emailOrPhone: string, password: string, username: string) => {
    ...
    try {
      ...
      if (!email.includes('@')) {
        ...
        safeSetItem('synergy_registering_phone', phone);
      }
  ...
  const checkout = async () => {
    ...
    const newOrder: Order = { 
        ...
        timeline: [
          { status: 'Payment Verified', date: formatThailandDate(getThailandNow(), { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }), completed: true }, 
    ...
    let socketData: { uplineId?: string; userName?: string; referrerTier?: UserTier; buyerTier?: UserTier } = {};
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        socketData.buyerTier = userData.tier;
        ...
        let lastTierInChain = userData.tier; // Start with buyer's tier
        const sameTierPaid = new Set<UserTier>();
        ...
        uplineChain.forEach((upline, depth) => {
            ...
            const rates = depth === 0 ? DIRECT_RATES[uplineData.tier] : INDIRECT_RATES[uplineData.tier];
            
            if (uplineData.tier === lastTierInChain) {
                // Check if this is a same-tier commission (one level only)
                if (!sameTierPaid.has(uplineData.tier)) {
                    commissionRate = rates?.[lastTierInChain] || 0;
                    if (commissionRate > 0) {
                        sameTierPaid.add(uplineData.tier);
                    }
                } else {
                    commissionRate = 0; // Already paid same-tier for this level
                }
            } else {
                // Different tier, all levels apply (differential logic)
                commissionRate = rates?.[lastTierInChain] || 0;
            }
            ...
            if (commissionRate > 0) {
                ...
                // Update public profile for real-time leaderboard (including pending)
                const today = getThailandTodayString();
                const lastDate = uplineData.lastIncomeDate;
                let dailyIncome = uplineData.dailyIncome || 0;
                
                if (lastDate !== today) {
                  dailyIncome = commissionAmount;
                } else {
                  dailyIncome = TO_PRECISION(dailyIncome + commissionAmount);
                }
                
                const newAccumulatedIncome = TO_PRECISION((uplineData.accumulatedIncome || 0) + commissionAmount);
                
                transaction.update(doc(db, 'publicProfiles', currentUplineId), {
                  dailyIncome,
                  lastIncomeDate: today,
                  accumulatedIncome: newAccumulatedIncome
                });

                // Also update users collection to keep accumulatedIncome, dailyIncome, and lastIncomeDate in sync
                transaction.update(doc(db, 'users', currentUplineId), {
                  accumulatedIncome: newAccumulatedIncome,
                  dailyIncome,
                  lastIncomeDate: today
                });
        ...
        set(liveSaleRef, {
          name: userData.name,
          amount: total,
          time: getThailandNow().toISOString()
        });
      });
      ...
      if (socket) {
        ...
        if (socketData.uplineId) {
          const commissionAmount = TO_PRECISION((total / 1.07) * (DIRECT_RATES[socketData.referrerTier || UserTier.STARTER]?.[socketData.buyerTier || UserTier.STARTER] || TIER_RATES[UserTier.STARTER]));
  ...
  const updateOrderStatus = async (orderId: string, status: OrderStatus, reason?: string) => { 
      try {
        ...
        const now = formatThailandDate(getThailandNow(), { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
        ...
        if (status === 'To Ship') {
            ...
            await updateDoc(orderRef, { deliveredAt: getThailandNow().toISOString() });
                ...
                if (tx.status === 'Paid') {
                    ...
                    await runTransaction(db, async (transaction) => {
                        ...
                        transaction.update(userRef, {
                            ...
                            accumulatedIncome: TO_PRECISION(Math.max(0, (userData.accumulatedIncome || 0) - tx.amount))
                        });
                        transaction.update(d.ref, { status: 'Pending' });
                    });
                } else {
                    await updateDoc(d.ref, { status: 'Pending' });
                }
            }
            // Create refund entry for buyer
            const refundAmount = Math.max(0, o.total - 50);
            const refundId = Date.now();
            await setDoc(doc(db, 'commissions', refundId.toString()), cleanData({
                id: refundId,
                userId: o.userId,
                orderId: orderId,
                date: now,
                source: `Refund: Order #${orderId} (Shipping -฿50)`,
                type: 'Refund',
                amount: refundAmount,
                status: 'Pending'
            }));
        } else if (status === 'Returned' || status === 'Cancelled') {
            updatedTimeline.push({ status: status === 'Returned' ? 'Returned & Refunded' : 'Order Cancelled', date: now, completed: true });
            
            if (status === 'Returned') {
                // Approve refund entry
                const qRefund = query(collection(db, 'commissions'), where('orderId', '==', orderId), where('type', '==', 'Refund'), where('status', '==', 'Pending'));
                const snapRefund = await getDocs(qRefund);
                for (const d of snapRefund.docs) {
                    const refundTx = d.data() as CommissionTransaction;
                    await runTransaction(db, async (transaction) => {
                        const userRef = doc(db, 'users', refundTx.userId);
                        const userSnap = await transaction.get(userRef);
                        if (!userSnap.exists()) return;
                        const userData = userSnap.data() as User;
                        transaction.update(userRef, { 
                            walletBalance: TO_PRECISION(userData.walletBalance + refundTx.amount)
                        });
                        transaction.update(d.ref, { status: 'Paid' });
                    });
                }
            }
  ...
  const reverseCommissionAndSales = async (orderId: string, orderTotal: number) => {
      try {
        ...
        const buyerUid = orderData.userId;

        const q = query(collection(db, 'commissions'), where('orderId', '==', orderId));
        const snapshot = await getDocs(q);
        
        // 1. Reverse buyer's accumulatedSales
        await runTransaction(db, async (transaction) => {
            const buyerRef = doc(db, 'users', buyerUid);
            const buyerSnap = await transaction.get(buyerRef);
            if (buyerSnap.exists()) {
                const bData = buyerSnap.data() as User;
                transaction.update(buyerRef, {
                    accumulatedSales: TO_PRECISION(Math.max(0, bData.accumulatedSales - orderTotal))
                });
            }
        });

        // 2. Reverse each commission
        for (const commissionDoc of snapshot.docs) {
          ...
          await runTransaction(db, async (transaction) => {
            ...
            if (userData.lastIncomeDate === today) {
                updates.dailyIncome = TO_PRECISION(Math.max(0, (userData.dailyIncome || 0) - tx.amount));
            }

            if (tx.status === 'Paid') {
                updates.walletBalance = TO_PRECISION(Math.max(0, userData.walletBalance - tx.amount));
            }

            transaction.update(userRef, updates);

            // Update public profile for leaderboard
            const publicRef = doc(db, 'publicProfiles', tx.userId);
            const publicSnap = await transaction.get(publicRef);
            if (publicSnap.exists()) {
                const publicData = publicSnap.data();
                const today = getThailandTodayString();
                const lastDate = publicData.lastIncomeDate;
                
                let dailyIncome = publicData.dailyIncome || 0;
                // Only deduct from daily income if the commission was from TODAY
                if (lastDate === today) {
                    dailyIncome = TO_PRECISION(Math.max(0, dailyIncome - tx.amount));
                }
                
                transaction.update(publicRef, {
                    dailyIncome,
                    lastIncomeDate: publicData.lastIncomeDate,
                    accumulatedIncome: TO_PRECISION(Math.max(0, (publicData.accumulatedIncome || 0) - tx.amount))
                });
            }
            
            transaction.update(commissionDoc.ref, { status: 'Cancelled' });
            
            const notifId = (Date.now() + Math.random()).toString();
            transaction.set(doc(db, 'notifications', notifId), cleanData({
                userId: tx.userId,
                title: "Commission Reversed",
                message: `Commission of ฿${tx.amount} for Order ${orderId} has been deducted due to product return.`,
                date: "Just now",
                type: 'system',
                read: false,
  ...
  const updateUserProfile = async (data: Partial<User>) => { 
    ...
    try {
      ...
      if (!checkDocSize(updatedUser)) return;
  ...
  const addReview = async (productId: number, orderId: string, rating: number, text: string, images: string[]) => {
      ...
      try {
        ...
        const newReview = { id: Date.now(), user: user.name, rating, text, date: formatThailandDate(getThailandNow()), images };
  ...
  const addReferrer = async (code: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      const isReferrerActive = referrerData.role === 'admin' || !!referrerData.referrerCode;
      if (!isReferrerActive) {
        return { success: false, error: 'This referral code is not yet active. The owner must link a referrer first.' };
      }

      // Increment teamSize of all ancestors in the uplinePath
      const uplinePath = [...(referrerData.uplinePath || []), referrerUid];
      ...
      await setDoc(doc(db, 'publicProfiles', auth.currentUser.uid), { uplinePath }, { merge: true });
  ...
  const updateProduct = async (productId: number, data: Partial<Product>) => {
    try {
      // Check full document size before update
      const existingProduct = products.find(p => p.id === productId);
      if (existingProduct) {
        const merged = { ...existingProduct, ...data };
        if (!checkDocSize(merged)) return;
      }
  ...
  const updateCommissionStatus = async (txId: number, status: CommissionTransaction['status']) => {
    try {
      const txRef = doc(db, 'commissions', String(txId));
      const txSnap = await getDoc(txRef);
      if (!txSnap.exists()) return;
      const tx = txSnap.data() as CommissionTransaction;

      // If a withdrawal is being cancelled, return funds to user
      if (tx.type === 'Withdrawal' && status === 'Cancelled' && tx.status !== 'Cancelled') {
        await runTransaction(db, async (transaction) => {
          const userRef = doc(db, 'users', tx.userId);
          const userSnap = await transaction.get(userRef);
          if (!userSnap.exists()) return;
          ...
          // amount is negative for withdrawals, so we subtract it to add it back
          transaction.update(userRef, {
            walletBalance: TO_PRECISION(userData.walletBalance - tx.amount)
          });
          transaction.update(txRef, { status: 'Cancelled' });
        });
      } else {
  ...
  const stopBroadcast = () => {
    setActivePromo(null);
    if (socket) {
      socket.emit("admin:stop_broadcast");
    }
  };
  ...
  const updateUserAdminProfile = async (userId: string, data: Partial<User>) => {
    ...
    try {
      ...
      const publicFields = ['name', 'avatar', 'tier', 'referralCode'];
  ...
  const healTeamSizes = async () => {
    ...
    try {
      ...
      allUsers.forEach(u => {
        if (u.uplinePath && Array.isArray(u.uplinePath)) {
          u.uplinePath.forEach(ancestorId => {
            teamSizeMap[ancestorId] = (teamSizeMap[ancestorId] || 0) + 1;
          });
        }
      });
  ...
  const seedInitialData = async () => {
    ...
    try {
      showToast({ message: "Seeding initial data...", type: 'info' });
      ...
      for (const ad of INITIAL_ADS) {
        await setDoc(doc(db, 'ads', String(ad.id)), cleanData(ad));
      }

      // Seed Campaign Assets
      for (const asset of INITIAL_ASSETS) {
        await setDoc(doc(db, 'campaignAssets', String(asset.id)), cleanData(asset));
      }

      // Seed Onboarding Slides
      for (const slide of INITIAL_ONBOARDING) {
        await setDoc(doc(db, 'onboardingSlides', String(slide.id)), cleanData(slide));
      }
      ...
      if (!settingsSnap.exists()) {
        await setDoc(doc(db, 'systemSettings', 'current'), {
          ...
          slipBackground: null,
          contactLinks: {
            line: 'https://line.me/ti/p/@synergy',
            phone: '+6620000000',
            email: 'support@synergyflow.com',
            website: 'https://synergyflow.com',
            terms: 'https://synergyflow.com/terms',
            privacy: 'https://synergyflow.com/privacy'
          }
        });
      }

      showToast({ message: "Initial data seeded successfully", type: 'success' });
      await refreshAllData();
    } catch (err) {
      console.error("Seeding failed", err);
      showToast({ message: "Seeding failed", type: 'error' });
    }
  };
  ...
  const refreshAllData = async () => {
    ...
    try {
      ...
      safeSetItem('synergy_cached_products', JSON.stringify(productsData));
      ...
      setFeed(feedData);
      safeSetItem('synergy_cached_feed', JSON.stringify(feedData));
      ...
      setAds(adsData);
      safeSetItem('synergy_cached_ads', JSON.stringify(adsData));
      ...
      safeSetItem('synergy_cached_campaignAssets', JSON.stringify(assetsData));
      ...
      safeSetItem('synergy_cached_onboardingSlides', JSON.stringify(onboardingData));
      ...
      if (auth.currentUser && user) {
        ...
        if (userSnap.exists()) {
          const userData = userSnap.data() as User;
          setUser(userData);
          safeSetItem(`synergy_cached_user_${auth.currentUser.uid}`, JSON.stringify(userData));
          safeSetItem(`synergy_last_user_doc_fetch_${auth.currentUser.uid}`, String(Date.now()));
        }
        ...
        safeSetItem('synergy_cached_orders', JSON.stringify(ordersData));
        ...
        safeSetItem('synergy_cached_commissions', JSON.stringify(commissionsData));
        ...
        safeSetItem('synergy_cached_notifications', JSON.stringify(notificationsData));
