    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (html5QrCode) {
                ...
                // Use a self-invoking function to handle async cleanup without blocking the return
                (async () => {
                    try {
                        if (scannerToStop.isScanning) {
                            await scannerToStop.stop();
                        }
                        // Only clear if the element still exists in the DOM to avoid errors
                        if (document.getElementById(containerId)) {
                            scannerToStop.clear();
                        }
                    } catch (err) {
                        // Silently fail on unmount cleanup to avoid console noise
                        // but log if it's a real issue during development
                        if (process.env.NODE_ENV === 'development') {
                            console.warn("Scanner cleanup on unmount:", err);
                        }
                    }
                })();
            }
