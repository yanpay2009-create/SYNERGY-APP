        ...
        <div className="relative w-full flex flex-col items-center">
            {activePromo.title && (
                <div className="mb-4 animate-in slide-in-from-top-4 duration-700">
                    <h2 className="text-2xl font-black text-white text-center uppercase tracking-[0.2em] drop-shadow-lg">
                        {activePromo.title}
                    </h2>
                </div>
            )}
            <div className="relative aspect-square w-full flex items-center justify-center p-2">
                {/* Close Button - Now inside the image area at top right */}
                <button 
                  onClick={dismissPromotion}
                  className="absolute top-4 right-4 z-20 p-2 bg-black/40 hover:bg-black/60 rounded-full text-white transition active:scale-90 border border-white/20 shadow-lg"
                >
                  <X size={24} />
                </button>
