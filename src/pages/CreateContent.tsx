...
export const CreateContent: React.FC = () => {
  ...
  const myContents = useMemo(() => {
    if (!user) return [];
    return feed.filter(f => f.userId === user.uid && f.isAi);
  }, [feed, user]);
    ...
    reader.onloadend = async () => {
        ...
        if (isVideo) {
            ...
            if (file.size > 50 * 1024 * 1024) { // 50MB limit for video
                showToast({ message: "Video file is too large. Please upload a video under 50MB.", type: 'error' });
                return;
            }
            
            // For videos, we use URL.createObjectURL to preview
            // We only convert to base64 if it's small enough for Firestore
            // Otherwise we warn the user or handle it differently
            // In this app, we'll stick to base64 for now but increase the threshold slightly
            // and add a compression warning if it's likely to fail Firestore's 1MB limit.
            ...
            videoElement.onloadedmetadata = () => {
                ...
                if (duration > 180) {
                    ...
                    // If video is > 1MB, it will fail Firestore's limit
                    if (file.size > 1024 * 1024) {
                        showToast({ message: "Video is large. AI analysis might be slower.", type: 'info' });
                    }
  ...
  return (
    <div className="pb-20 pt-0 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div className="flex p-1 bg-gray-100 dark:bg-gray-800 rounded-2xl mb-6">
          <button 
              onClick={() => setShowContentList(false)}
              className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!showContentList ? 'bg-white dark:bg-gray-700 text-synergy-blue shadow-sm' : 'text-gray-400'}`}
          >
              Create Content
          </button>
          <button 
              onClick={() => setShowContentList(true)}
              className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all relative ${showContentList ? 'bg-white dark:bg-gray-700 text-synergy-blue shadow-sm' : 'text-gray-400'}`}
          >
              Manage Assets
          </button>
      </div>
      ...
      {!showContentList ? (
        <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div 
              ...
              className={`w-full aspect-square rounded-[32px] border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition relative overflow-hidden group ${media ? 'border-transparent' : 'border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50'}`}
          >
              {media ? (
                  <>
                      {mediaType === 'video' ? (
                      ...
                      {mediaType === 'video' && (
                          <div className="absolute bottom-3 left-3 flex items-center space-x-2">
                              <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-white text-[10px] font-black flex items-center space-x-1 border border-white/20">
                                  <Clock size={12} />
                                  <span>{videoDuration.toFixed(1) || 0}s / 180s</span>
                              </div>
        ...
        <div className="animate-in fade-in duration-500">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[32px] shadow-soft dark:shadow-none border border-transparent dark:border-gray-700 mb-6">
                <div className="flex space-x-2 overflow-x-auto no-scrollbar mb-6">
                    {['All', 'Approved', 'Pending'].map((tab) => (
                        <button 
                            ...
                            className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${contentTab === tab ? 'bg-synergy-blue text-white shadow-md' : 'bg-gray-50 dark:bg-gray-700 text-gray-400'}`}
                        >
                            {tab}
                        </button>
                    ))}
                ...
                <div className="space-y-4">
                    {filteredMyContents.length === 0 ? (
                        <div className="text-center py-20 bg-gray-50 dark:bg-gray-900/50 rounded-[32px] border border-dashed border-gray-200 dark:border-gray-700">
                        ...
                        filteredMyContents.map(post => {
                            ...
                            return (
                                <div key={post.id} className={`bg-gray-50 dark:bg-gray-900/50 rounded-[28px] overflow-hidden border border-transparent transition-all duration-500 ${isExpanded ? 'ring-2 ring-synergy-blue/20' : 'hover:border-gray-200 dark:hover:border-gray-700'}`}>
                                    <div className="p-4" onClick={() => setExpandedPostId(isExpanded ? null : post.id)}>
                                        <div className="flex justify-between items-center mb-4">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <button 
                                                    ...
                                                    className="font-mono text-[10px] font-black text-gray-900 dark:text-gray-100 flex items-center space-x-1.5"
                                                >
                                            ...
                                            <div className="flex items-center space-x-2">
                                                <span className={`text-[8px] px-2.5 py-1 rounded-full font-black uppercase tracking-[0.15em] ${statusColor}`}>
                                                    {post.status}
                                                </span>
                                        ...
                                        <div className="flex items-center space-x-3 mb-4">
                                            <div className="relative w-16 aspect-square shrink-0">
                                                {post.type === 'video' ? (
                                                    ...
                                                    <img src={post.content || undefined} className="w-full h-full object-cover rounded-xl border border-gray-100 dark:border-gray-800" alt="content" />
                                                )}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h3 className="text-xs font-black text-gray-900 dark:text-white truncate mb-1 leading-tight">{post.caption.split('\n')[0] || 'Untitled Creation'}</h3>
                                                <div className="flex items-center space-x-2 text-gray-400 mb-2">
                                                    <Calendar size={10} />
                                                    <p className="text-[9px] font-bold uppercase tracking-tight">Modified 2h ago</p>
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    <p className="text-[9px] font-black text-synergy-blue uppercase tracking-[0.1em]">{post.type}</p>
                                                    <div className="h-2 w-[1px] bg-gray-200 dark:bg-gray-800" />
                                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.1em]">{post.mood || 'Standard'}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex space-x-2 pt-3 border-t border-gray-100 dark:border-gray-800">
                                            <button 
                                                ...
                                                className="flex-1 py-2.5 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-center space-x-2 active:scale-95 transition"
                                            >
                                                <Edit3 size={14} />
                                                <span>Edit</span>
                                            </button>
                                            <button 
                                                ...
                                                className="w-10 h-10 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 flex items-center justify-center active:scale-95 transition border border-red-100 dark:border-red-900/30"
                                            >
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    ...
                                    {isExpanded && (
                                        <div className="bg-white dark:bg-gray-800/40 p-4 space-y-4 border-t border-gray-100 dark:border-gray-800 animate-in slide-in-from-top-3 duration-500">
                                            <div className="space-y-2">
                                                ...
                                                <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                                                    <p className="text-[10px] text-gray-700 dark:text-gray-300 font-medium leading-relaxed whitespace-pre-wrap">
                                                        {post.caption}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            {post.productId && (
                                                <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                                                    ...
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <p className="text-[9px] font-black text-gray-900 dark:text-white truncate">{products.find(p => p.id === post.productId)?.name}</p>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
