...
export const Account: React.FC = () => {
  ...
  const todayString = useMemo(() => {
    return formatThailandDate(getThailandNow(), { day: '2-digit', month: 'short', year: 'numeric' });
  }, []);
  
  const preciseDailyEarnings = useMemo(() => {
    if (!isLoggedIn || !user) return 0;
    
    // Check if the user's daily income is from today
    const today = getThailandTodayString();

    if (user.lastIncomeDate === today) {
      return user.dailyIncome || 0;
    }
    
    return 0;
  }, [isLoggedIn, user?.dailyIncome, user?.lastIncomeDate]);
  ...
  useEffect(() => {
    ...
    safeSetItem('synergy_privacy_mode', showEarnings.toString());
  ...
  if (!isLoggedIn || !user) {
    return (
      <div className="min-h-screen bg-transparent flex flex-col items-center justify-center px-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in slide-in-from-bottom-12 duration-500 mb-20">
          <Auth />
        </div>
      </div>
    );
  ...
  return (
    <div className="pb-0 pt-0 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 relative overflow-x-hidden">
      ...
      <ReferrerModal 
        ...
        description="To activate your affiliate profile and start sharing, you must link your account to a referrer code first."
      />
            ...
            <div className="mb-5 animate-in slide-in-from-bottom-2">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-[24px] overflow-hidden shadow-soft border border-gray-100 dark:border-gray-700 relative">
                <div className="relative z-10">
                    ...
                    <MenuRow icon={Megaphone} label="Marketing Campaigns" subtitle="Coordinate marketing and promo assets" to="/admin/campaign-assets" colorClass="bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400" navigate={navigate} triggerPinGate={triggerPinGate} isLast={false} />
                    <MenuRow icon={Layout} label="Advertising Assets" subtitle="Optimize platform visibility and ads" to="/admin/ads" colorClass="bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400" navigate={navigate} triggerPinGate={triggerPinGate} isLast={false} />
                    <MenuRow icon={Monitor} label="Onboarding Experience" subtitle="Refine user onboarding journey" to="/admin/onboarding" colorClass="bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400" navigate={navigate} triggerPinGate={triggerPinGate} isLast={true} />
                </div>
              </div>
            </div>
          )}
