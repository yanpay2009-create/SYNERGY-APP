...
export const SocialAccounts: React.FC = () => {
  ...
  const SocialRow = ({ platform, icon: Icon, label, colorClass, isLast }: any) => {
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div className="text-left">
             <span className="text-sm font-bold text-gray-700 dark:text-gray-200 leading-tight">{label}</span>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          {data.connected ? (
            <span className="text-[11px] font-bold text-emerald-500">{data.name || 'Linked'}</span>
          ) : (
            <span className="text-[11px] font-bold text-gray-400">Not Linked</span>
          )}
        </div>
      </button>
    );
    ...
    <div className="pb-24 pt-0 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      <div className="sticky top-0 z-[100] bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-100/50 dark:border-gray-800/50 -mx-4 px-4 py-3 mb-6 transition-all">
        <div className="flex items-center">
          ...
          <h1 className="text-xl font-bold ml-2 text-gray-900 dark:text-white tracking-tight">Social Accounts</h1>
        </div>
      </div>
      ...
      <div className="animate-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white dark:bg-gray-800 rounded-[24px] overflow-hidden shadow-soft border border-gray-100 dark:border-gray-700 relative">
          <SocialRow 
            ...
            icon={Facebook} 
            label="Facebook" 
            colorClass="bg-gray-100 dark:bg-gray-700 text-[#1877F2]" 
            isLast={false}
          />
          <SocialRow 
            platform="line" 
      ...
      <div className="mt-10 p-6 bg-white dark:bg-gray-800 rounded-[32px] border border-gray-100 dark:border-gray-700 shadow-soft">
