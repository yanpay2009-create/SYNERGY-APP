...
export const ProductDetail: React.FC = () => {
  const { products, addToCart, calculateCommission, user, referrer, addReferrer, t, setBottomNavHidden, cart, toggleFavorite, isFavorite, showToast, isLoggedIn, allOrders } = useApp();
  ...
  const relevantOrder = (user && isLoggedIn) ? allOrders.find((o: any) => 
    o.userId === user.uid && 
    o.status === 'Delivered' && 
    o.items.some((item: any) => item.id === product.id) &&
    !o.reviewedProductIds?.includes(product.id)
  ) : null;
  ...
  return (
    <div className="pb-24 pt-0 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="relative px-2 py-2 z-20">
            ...
            <div className="flex justify-between items-end mb-6 border-b border-gray-100 dark:border-gray-800 pb-5">
                <div>
                    ...
                    {hasDiscount && (
                        <div className="text-synergy-blue text-[10px] font-black tracking-tight mt-1">
                            {user?.tier === UserTier.MARKETER ? '10 % Personal Discount' : 
                             user?.tier === UserTier.BUILDER ? '20 % Personal Discount' : 
                             user?.tier === UserTier.EXECUTIVE ? '30 % Personal Discount' : ''}
                        </div>
                    )}
                </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex flex-col">
                            ...
                            {relevantOrder && (
                                <button 
                                    onClick={() => navigate(`/write-review/${relevantOrder.id}/${product.id}`)}
                                    className="text-[10px] text-emerald-600 font-black uppercase tracking-tighter mt-1 flex items-center space-x-1 animate-pulse"
                                >
                                    <Sparkles size={10} />
                                    <span>You bought this! Write a review</span>
                                </button>
                            )}
                        </div>
                        <button 
      ...
      <ReferrerModal 
        ...
        description="To generate your personal affiliate link and track commissions, you must link your account to a referrer."
      />
