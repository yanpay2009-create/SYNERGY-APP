    ...
    <div className="pb-20 pt-0 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 relative transition-colors duration-300 font-sans">
      ...
      <div className={`fixed top-0 left-0 right-0 z-[100] max-w-md mx-auto transition-all duration-300 ease-in-out ${
        scrolled || homeAds.length === 0
          ? 'bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl py-3 px-4 shadow-sm'
        ...
        <div className="flex items-center space-x-3">
          <div className={`flex-1 flex items-center px-4 py-2 rounded-2xl border transition-all duration-300 ease-in-out ${
            ...
            <Search size={16} className={`transition-colors duration-300 ${scrolled || homeAds.length === 0 ? 'text-gray-400' : 'text-white/90'} mr-2`} />
            <input 
              type="text" 
              value={searchQuery}
              ...
              className={`bg-transparent border-none outline-none text-xs w-full font-medium transition-colors duration-300 ${
                scrolled || homeAds.length === 0 ? 'text-gray-800 dark:text-white placeholder-gray-400' : 'text-white placeholder-white/90'
              }`}
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className={`transition-colors duration-300 ${scrolled || homeAds.length === 0 ? 'text-gray-400' : 'text-white/70'} hover:text-white`}>
                <X size={14} />
              </button>
            )}
          </div>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className={`absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border transition-colors duration-300 ${
                scrolled || homeAds.length === 0 ? 'border-white dark:border-gray-900' : 'border-white'
              }`}></span>
            )}
          </button>
        </div>
      ...
      <div className={`relative z-10 bg-gray-50 dark:bg-gray-900 pt-6 px-4 min-h-screen ${homeAds.length > 0 ? '-mt-4 shadow-[0_-20px_40px_rgba(0,0,0,0.05)]' : ''}`}>
          ...
          <>
            ...
            <div className="mb-3">
              {homeBannerAds.length > 0 && homeBannerAds[0].image ? (
                ...
                >
                  <div className="w-full h-32 rounded-2xl overflow-hidden shadow-soft border border-gray-100 dark:border-gray-700">
                    <img 
                ...
                >
                  <div 
                    className={`w-full h-32 rounded-xl ${tierColors.bgLight} backdrop-blur-xl p-6 flex flex-col justify-center shadow-soft dark:shadow-none border border-white/60 dark:border-gray-700 relative overflow-hidden group transition-all duration-500`}
                  >
                    {/* Content Overlay */}
                    <div className="relative z-10">
                      <div className="flex items-start space-x-3 mb-1.5">
                        <div className={`p-3 ${tierColors.bgLight} backdrop-blur-md rounded-full border border-white/50 dark:border-gray-600 shadow-inner mt-0.5`}>
                          <Store size={32} className={tierColors.text} />
                        </div>
                        <div className="flex flex-col">
                          <h3 className="text-lg font-black tracking-tight leading-tight">
                            <span className="text-gray-900 dark:text-white">SYNERGY</span>
                            <span className="text-synergy-blue ml-1.5">FLOW</span>
                          </h3>
                          <p className="text-[10px] text-gray-500 dark:text-gray-400 font-bold line-clamp-1">Affiliate Earn up to 30% Commission</p>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        ...
                        <div className="flex -space-x-3">
                          {[1, 2, 3].map(i => (
                            <div key={i} className={`w-8 h-8 rounded-full border-2 border-white/50 dark:border-gray-600 ${tierColors.bgLight} backdrop-blur-sm flex items-center justify-center shadow-sm`}>
            ...
            <div className="mb-4">
                <div className="flex items-center space-x-2 mb-3">
                    <TrendingUp className="text-synergy-blue" size={20} />
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t('home.affiliate_progress')}</h3>
                </div>
                <button onClick={() => navigate('/tier-benefits')} className={`w-full text-left ${tierColors.bgLight} backdrop-blur-xl rounded-xl p-6 shadow-soft dark:shadow-none border border-white/60 dark:border-gray-700 relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all duration-200`}>
