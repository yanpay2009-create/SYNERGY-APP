...
export const Notifications: React.FC = () => {
  const { notifications, markNotificationAsRead, t, user, allCommissions, allOrders, broadcastPromotion, systemSettings } = useApp();
  const navigate = useNavigate();
  const location = useLocation();
  const [selectedNotif, setSelectedNotif] = useState<Notification | null>(null);

  // Filter notifications to show only necessary ones
  // 1. All global notifications (promotions, system updates)
  // 2. Personal notifications (orders, commissions)
  // 3. Filter out any potential duplicates or irrelevant system logs
  const filteredNotifications = useMemo(() => {
    return notifications.filter(notif => {
      // Always show global notifications
      if (notif.userId === 'global') return true;
      
      // Show order and commission related notifications for the specific user
      // Check relatedType or title for commission/order relevance
      if (notif.type === 'order' || notif.relatedType === 'order') return true;
      if (notif.relatedType === 'commission') return true;
      
      // Show promo notifications
      if (notif.type === 'promo' || notif.relatedType === 'promo') return true;

      // Filter out generic info notifications unless they have a title/message
      if (notif.title && notif.message) return true;
      ...
      <div className="sticky top-0 z-[100] bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-100/50 dark:border-gray-800/50 -mx-4 px-4 py-3 mb-6 transition-all shadow-2xl dark:shadow-none">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold ml-2 text-gray-900 dark:text-white tracking-tight">Notifications</h1>
          </div>
          <button 
            className="text-[10px] font-black uppercase tracking-widest text-synergy-blue bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded-full border border-blue-100 dark:border-blue-800 active:scale-95 transition"
