...
export const AdminDashboard: React.FC = () => {
  ...
  const { 
    ...
    broadcastPromotion, stopBroadcast, dismissPromotion, updateOrderStatus, deleteOrder, 
    ...
    updateUserReferralCode, factoryReset, healReferralCodes, healPhoneMap, healTeamSizes, healUplinePaths, seedInitialData, updateUserRole
  } = useApp();
    ...
    if (previewPost?.type === 'video' && videoRef.current) {
        ...
        const playVideo = async () => {
            ...
            try {
                ...
                // Silently ignore AbortError (interrupted play) or media removal errors
                const isInterrupted = err instanceof Error && (
                    err.name === 'AbortError' || 
  ...
  const pendingKycCount = useMemo(() => allTeamMembers.filter(m => m.kycStatus === 'Pending').length, [allTeamMembers]);
  ...
  const filteredList = useMemo(() => {
    ...
    if (activeTab === 'Orders') {
      return allOrders.filter(o => {
        ...
        if (statusFilter === 'Returns') matchesStatus = o.status === 'Return Pending';
        if (statusFilter === 'Returned') matchesStatus = o.status === 'Returned';
        return matchesQuery && matchesStatus;
      });
    }
  ...
  const filterOptions = useMemo(() => {
    ...
    if (activeTab === 'Orders') return ['All', 'Pending', 'To Ship', 'Shipped', 'Delivered', 'Returns', 'Returned'];
  ...
  const handleListUpdateOrderStatus = async (id: string, next: OrderStatus) => {
      await updateOrderStatus(id, next);
      showToast({ message: `Order status updated to ${next}`, type: 'success' });
      if (next === 'To Ship') {
          const order = allOrders.find(o => o.id === id);
    ...
    <div className="pb-24 pt-4 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 font-sans">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          >
              <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${pendingKycCount > 0 ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                          <ShieldCheck size={16} />
                      </div>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">KYC Pending</span>
                  </div>
                  {pendingKycCount > 0 && <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping"></span>}
              </div>
              <p className={`text-2xl font-black ${pendingKycCount > 0 ? 'text-amber-600' : 'text-gray-900 dark:text-white'}`}>{pendingKycCount}</p>
              <p className="text-[10px] text-gray-400 mt-1">Identity Audits</p>
          </div>
          ...
          >
              <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${pendingPostsCount > 0 ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                          <Sparkles size={16} />
                      </div>
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pending Posts</span>
                  </div>
                  {pendingPostsCount > 0 && <span className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-ping"></span>}
              </div>
              <p className={`text-2xl font-black ${pendingPostsCount > 0 ? 'text-purple-600' : 'text-gray-900 dark:text-white'}`}>{pendingPostsCount}</p>
              <p className="text-[10px] text-gray-400 mt-1">Content Queue</p>
          </div>
      </div>
            ...
            <div className="space-y-4">
                {paginatedList.length === 0 ? (
                    ...
                    paginatedList.map((item: any) => {
                        if (activeTab === 'Members') {
                            return (
                              <div key={item.id} className="flex items-center justify-between pb-5 border-b border-gray-50 dark:border-gray-700 last:border-0 last:pb-0 transition-all">
                        ...
                        if (activeTab === 'Orders') {
                            return (
                              <div key={item.id} className="flex items-center justify-between pb-5 border-b border-gray-50 dark:border-gray-700 last:border-0 last:pb-0 transition-all">
                                  <div className="flex items-center space-x-4 min-w-0">
                                      <div className="w-11 h-11 rounded-[18px] overflow-hidden shrink-0 border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
                                          {item.items && item.items[0] ? (
                                              <img src={item.items[0].image} alt="Product" className="w-full h-full object-cover" />
                                          ) : (
                                      ...
                                      <div className="min-w-0">
                                          ...
                                          <div className="relative mt-1" onClick={(e) => e.stopPropagation()}>
                                              <select 
                                                  value={item.status}
                                                  onChange={(e) => handleListUpdateOrderStatus(item.id, e.target.value as OrderStatus)}
                                                  className="appearance-none bg-transparent text-[9px] text-synergy-blue font-black uppercase tracking-widest pr-4 focus:outline-none cursor-pointer"
                                              >
                                                  {['Pending', 'To Ship', 'Shipped', 'Delivered', 'Return Pending', 'Returned'].map(status => (
                                  ...
                                  <div className="flex items-center space-x-3">
                                      ...
                                      <button 
                                          onClick={(e) => handleDeleteOrder(e, item.id)} 
                                          className="p-2.5 text-red-500 bg-red-50 dark:bg-red-900/20 rounded-2xl active:scale-95 transition-all"
                                      >
                                          <Trash2 size={16} />
                                      </button>
                                  </div>
                        ...
                        if (activeTab === 'Posts') {
                            return (
                                <div key={item.id} onClick={() => { setPreviewPost(item); setIsCaptionExpanded(false); }} className="flex items-center justify-between pb-5 border-b border-gray-50 dark:border-gray-700 last:border-0 last:pb-0 cursor-pointer active:opacity-70 transition-all">
                                    <div className="flex items-center space-x-4 min-w-0">
                                        <div className="w-11 h-11 bg-black rounded-2xl overflow-hidden shrink-0 border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-center">
                                            {item.type === 'video' ? <Play size={16} className="text-white" /> : <img src={item.content || undefined} className="w-full h-full object-cover" alt="p" />}
                                        </div>
                                        <div className="min-w-0">
                                            <p className="text-sm font-black text-gray-900 dark:text-white truncate tracking-tight">{item.user}</p>
                                            <p className="text-[10px] text-gray-400 truncate line-clamp-1 mt-0.5">{item.caption}</p>
                                            <div className="flex items-center space-x-2 mt-1">
                                                <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${item.status === 'Pending' ? 'bg-amber-50 text-amber-500 border border-amber-100' : 'bg-emerald-50 text-emerald-500 border border-emerald-100'}`}>
                                                    {item.status}
                                                </span>
                                            </div>
                                        </div>
                        ...
                        if (activeTab === 'KYC') {
                            return (
                                <div key={item.id} onClick={() => setSelectedKyc(item)} className="flex items-center justify-between pb-5 border-b border-gray-50 dark:border-gray-700 last:border-0 last:pb-0 cursor-pointer active:opacity-70 transition-all">
                                    <div className="flex items-center space-x-4 min-w-0">
                                        <img src={item.avatar || undefined} className="w-11 h-11 rounded-2xl object-cover border border-gray-100 dark:border-gray-700 shadow-sm" alt="u" />
                                        <div className="min-w-0">
                                            <p className="text-sm font-black text-gray-900 dark:text-white truncate tracking-tight">{item.name}</p>
                                            <p className="text-[10px] text-gray-400 truncate">{item.email || `user_${item.referralCode || item.id}@example.com`}</p>
                                            <div className="flex items-center space-x-2 mt-1">
                                                <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                                                    item.kycStatus === 'Verified' ? 'bg-emerald-50 text-emerald-500 border border-emerald-100' : 
                                                    item.kycStatus === 'Pending' ? 'bg-amber-50 text-amber-500 border border-amber-100' : 
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between pt-4 pb-2">
                  ...
                  <div className="flex items-center space-x-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    <span>{(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredList.length)}, {filteredList.length}</span>
                  </div>
